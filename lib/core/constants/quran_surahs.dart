/// قائمة سور القرآن الكريم الكاملة
class QuranSurahs {
  ...
  /// الحصول على اسم السورة برقمها
  static String getSurahName(int number) {
    final surah = getSurahByNumber(number);
    return surah?['name'] ?? '';
  }

  /// الحصول على عدد آيات السورة
  static int getVersesCount(int number) {
    final surah = getSurahByNumber(number);
  ...
  static List<Map<String, dynamic>> searchSurahs(String query) {
    ...
    return surahs.where((s) => 
      (s['name'] as String).contains(query) ||
      (s['number'] as int).toString() == query
    ).toList();
  }
}
